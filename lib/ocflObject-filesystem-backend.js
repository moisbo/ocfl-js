...
class OcflObject extends OcflObjectParent {
      ...
      try {
        if (await pathExists(this.repositoryPath)) {
          if (this.updateMode === "update") {
            ...
            await this.syncDepositToRepository({
  ...
  async verify() {
    ...
    for (let file of files) {
      ...
      let result = await check(file, hash);
    ...
    let result = await checkPath({ target: self.activeObjectPath });
    errors = [...errors, ...result.errors];
    if (!result.errors.length && (await pathExists(this.depositPath))) {
      result = await checkPath({
        target: self.depositPath,
      });
      errors = [...errors, ...result.errors];
    }
    ...
    async function check(file, hash) {
      let errors = [];
      let exists = await pathExists(file);
      if (exists) {
        if ((await self.__hash_file(file)) !== hash) {
    ...
    async function checkPath({ target }) {
      ...
      for (let file of filesFound) {
        const match = files.filter((f) => path.join(target, f.path) === file);
        if (!match.length) {
          isValid = false;
          errors.push(
  ...
  async syncDepositToRepository({ depositPath, repositoryPath }) {
    let paths = [];
    const id = this.id;
