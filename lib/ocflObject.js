...
class OcflObject {
  ...
  async commit({ inventory }) {
    ...
    if (this.activeObjectPath === this.depositPath) {
      // we broke out of the update so redo the inventories
      // const latestVersion = this.getLatestVersion();
      // ({
      //   lastVersionInventory,
      //   currentVersionInventory,
      // } = await this.__generateInventories({
      //   version: latestVersion.version,
      //   target: this.depositPath,
      // }));
  ...
  async isObject() {
    ...
    return await fs.pathExists(path.join(this.activeObjectPath, this.namaste));
  }
  ...
  async load() {
    ...
    let stats;
    try {
      stats = await fs.stat(this.activeObjectPath);
    } catch (error) {
      throw new Error(
        `${this.activeObjectPath} does not exist or is not a directory`
      );
    }
    if ((await fs.pathExists(this.activeObjectPath)) && stats.isDirectory()) {
  ...
  async remove() {
    await fs.remove(this.activeObjectPath);
    return null;
  }
