...
class OcflObject {
  ...
  // importContent is passed an id and a callback - this takes one argument, the
  ...
  async importContent(id, writeContent) {
    // Copy files into v1
  ...
  async load(path) {
    // Tries to load an exsiting object residing at <path>
    if (this.path) {
      throw new Error("This object has already been initialized at: " + this.path)
    }
    this.path = path;
