...
describe.only("Testing object creation functionality", () => {
  let object;
  const ocflRoot = "test-ocflObject-output";
  const id = "1";
...
describe("Testing object manipulation functionality - object with one version", () => {
  ...
  test(`should be able to resolve a file path relative to the object`, async () => {
    ...
    expect(file).toEqual("test-output/2/v1/content/sample/file_0.txt");
  });
});
