const { pathExists, stat, readdir, writeFile } = require("fs-extra");
...
class Repository extends EventEmitter {
  ...
  async __findObjectsS3Repository() {
    ...
    while (walk) {
      let content = await this.bucket.listObjects({ continuationToken });
      content.Contents.forEach((entry) => {
        if (entry.Key.match(/.*\/0=ocfl_object_1.0/)) {
          this.emit("object", { objectPath: entry.Key.split("/")[0] });
        }
      });
      if (content.ContinuationToken) {
