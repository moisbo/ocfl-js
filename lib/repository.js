...
class Repository {
  ...
  async findObjects(dir, objects) {
    ...
    for (let d of dirs) { 
      ...
      if (d != "deposit" && await fs.pathExists(potential_dir) && stats.isDirectory()){
          ...
          const objectNamastePath = path.join(potential_dir, "0=" + object.nameVersion(this.version));
  ...
  async isContentRoot(aPath) {
    ...
    const namastePath = path.join(aPath, "0=" + this.nameVersion(this.version));
    console.log("HERE", namastePath, await fs.pathExists(namastePath))
