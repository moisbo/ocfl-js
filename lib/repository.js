...
class Repository {
  ...
  async add_object_from_dir(dir, id) {
    ...
    console.log("Path in repo", objectRepoPath)
  ...
  async mergeObjectWith(newObject, prevObjectPath) {
    ...
    for (let hash of Object.keys(newState)) {
      ...
      if (prevState[hash]) {
        // Check that all the files we have here are in the previous one
        var newFileList = []
        for (let file of newState[hash]) {
          const fileInPrevState = prevState[hash].indexOf(file)
          if (fileInPrevState) {
            newFileList.push(prevState[hash][fileInPrevState]);
          }
          else {
            newFileList.push(file)
          }
          newState[hash] = newFileList;
        }
      }
    ...
    //const kill = await fs.remove(newObject.path)
    // Write the invtory into the prevObject
