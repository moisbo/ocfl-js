...
class Repository {
    ...
    if (await fs.pathExists(this.path) && stats.isDirectory()) {
      ...
      if (readDir.length <= 0) { // empty so initialise a repo here
        const generateNamaste = await this.generateNamaste(this.path, this.ocflVersion);
          ...
          throw new Error('can\'t initialise a repository as there are already files.' )
        }
      }
    else {
  ...
  async findObjects(dir, objects) {
    ...
    for (let d of dirs) {
      ...
      if (d != DEPOSIT_DIR && await fs.pathExists(potential_dir) && stats.isDirectory()) {
  ...
  async createNewObjectContent(id, writeContent) {
    ...
    if (!await fs.pathExists(objectRepoPath)) {
      var subPath = objectRepoPath
      do {
        subPath = path.join(subPath, "..");
        if (await object.isObject(subPath)) {
          throw new Error("Cannot make an object. There is already an object in a higher level directory.")
        }
      } while (subPath.startsWith(this.path));
      await object.removeEmptyDirectories();
      await fs.move(objectDepositPath, objectRepoPath);
    } else {
      // Directory exists, it might be our object but it might be on a path of an existing one (which is bad)
      if (!await object.isObject(objectRepoPath)) {
        throw new Error("There is no object here but the path already exists is this ID a subset of a longer one?")
      }
      // Merge object
