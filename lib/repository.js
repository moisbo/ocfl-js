...
class Repository {
  ...
  async export(id, exportPath, options) {
    ...
    const objToExport = new OcflObject(objectPath);
    objToExport.init();
  ...
  async add_object_from_dir(dir, id) {
    ...
    const objectDepositPath = path.join(this.path, "deposit", this.objectIdToPath(id)).replace("/","-");
    if (await fs.pathExists(objectDepositPath)) {
        ...
        shell.mkdir("-p", objectDepositPath);
    } 
    ...
    const newObj = await new OcflObject(objectRepoPath);
    const newoi = await newObj.init();
